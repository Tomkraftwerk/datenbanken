...
for ($i = 0; $i <= $n; $i++) {
  ...
  $person_entries .= ('("'.$i.'","'.$firstname.'","'.$lastname.'","'.$email.'","'.$city.'","'.$gender.'","'.$age.'")');
  if ($i < $n) {
    $person_entries .= ',';
  }
...
$query = ('INSERT INTO Persons (Person_Id, FirstName, LastName, Email, City, Gender, Age) VALUES '.$person_entries.';');
mysqli_query($con, $query);
