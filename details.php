...
if (isset($_GET["id"])){
    # Abruf und Anzeige der Datenbank-Inhalte
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                echo ('Wunsch erfüllen: <a href="mailto:erfuellen@wunschstern.de?subject=Wunsch%20Nr.'.$row["Wunsch_Id"].'%20erf%C3%BCllen">📧</a>');
            }
            else {
...
if (isset($_POST["intern"])){
        # Abruf und Anzeige der Datenbank-Inhalte
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                echo ('<form method="post" action="php/erfuellt.php">
                                <button type="submit" name="erfuellt" value="'.$_POST["intern"].'">Wunsch als erfüllt markieren</button>
                            </form>');
            }
            else {
                echo "Kein Eintrag vorhanden!";
